...
public:
    ...
    void runTests() {
        ...
        // 測試基本碰撞模型
        TestResult basicResult = runBasicCollisionTest();
        
        // 測試 OGC 模型
        TestResult ogcResult = runOGCCollisionTest();
        
        // 比較結果
        compareResults(basicResult, ogcResult);
        
        // 保存結果
        saveResults(basicResult, ogcResult);
...
private:
    TestResult runBasicCollisionTest() {
        ...
        simulation->setUseOGC(false); // 禁用 OGC
    ...
    void compareResults(const TestResult& basic, const TestResult& ogc) {
        ...
        // 計算標準差
        double basicStdDev = calculateStdDev(basic.frameTimes);
        double ogcStdDev = calculateStdDev(ogc.frameTimes);
    ...
    void saveResults(const TestResult& basic, const TestResult& ogc) {
        std::ofstream file("performance_test_results.csv");
        if (!file.is_open()) {
        ...
        file.close();
        std::cout << "\n結果已保存到 performance_test_results.csv" << std::endl;
    }
...
int main(int argc, char *argv[])
{
    ...
    std::cout << "=== OGC 布料模擬性能測試 ===" << std::endl;
